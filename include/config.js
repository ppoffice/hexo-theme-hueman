...
function loadThemeConfig(hexo, cfgPaths) {
  const configs = cfgPaths
    .map((cfgPath) => fs.readFileSync(cfgPath))
...
function checkConfig(hexo) {
  if (!process.argv.includes('--hueman-dont-check-config')) {
    logger.info('=== Checking theme configurations ===');

    const siteCfgFile = path.join(hexo.base_dir, '_config.yml');
    const themeSiteCfg = path.join(hexo.base_dir, '_config.hueman.yml');
    ...
    if (!process.argv.includes('--hueman-dont-generate-config')) {
      if (!themeCfgPaths.length) {
        ...
        logger.info('To skip configuration generation, use "--hueman-dont-generate-config".');
      }
    }

    let cfg = loadThemeConfig(hexo, themeCfgPaths);

    if (!process.argv.includes('--hueman-dont-upgrade-config')) {
      ...
      if (cfg.version && migrator.isOudated(cfg.version)) {
        logger.warn(
          `Your theme configuration is outdated (${cfg.version} < ${migrator.getLatestVersion()}).`,
        );
        logger.info('To skip the configuration upgrade, use "--hueman-dont-upgrade-config".');
    ...
    if (validation !== true) {
      ...
      logger.warn('hueman may still run, but you will encounter unexcepted results.');
...
module.exports = (hexo) => {
  try {
    ...
    logger.info("You may use '--hueman-dont-check-config' to skip configuration checking.");
    process.exit(-1);
  }
};
