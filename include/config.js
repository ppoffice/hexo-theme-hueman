  ...
  if (!process.argv.includes('--hueman-dont-check-config')) {
    ...
    if (!process.argv.includes('--hueman-dont-generate-config')) {
      if (!fs.existsSync(CONFIG_PATH)) {
        ...
        logger.info(
          "You may add '--hueman-dont-generate-config' to prevent creating the configuration file.",
        );
    ...
    try {
        ...
        if (migrator.isOudated(cfg.version)) {
          logger.info(
            `Your configuration file is outdated (${
              cfg.version
            } < ${migrator.getLatestVersion()}). ` + 'Trying to upgrade it...',
          );
          // Backup old config
          ...
          logger.info(
            `We also created an example at ${CONFIG_PATH + '.example'} for your reference.`,
          );
        }
      }
      ...
      logger.error("Please add '--hueman-dont-check-config' to your Hexo command if you");
